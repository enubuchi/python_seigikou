...
#条件に応じてifを分岐が解決策かと思われたが、辞書を使う手もある。
#https://www.pasonatech.co.jp/workstyle/column/detail.html?p=2635

if checksum==1:
    df[["High"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==2:
    df[["Low"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==3:
    df[["High","Low"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==4:
    df[["Open"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==5:
    df[["High","Open"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==6:
    df[["Low","Open"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==7:
    df[["High","Low","Open"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==8:
    df[["Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==9:
    df[["High","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==10:
    df[["Low","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==11:
    df[["High","Low","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==12:
    df[["Open","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==13:
    df[["High","Open","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==14:
    df[["Low","Open","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
elif checksum==15:
    df[["High","Low","Open","Close"]].head(90).plot(figsize=(16,8),fontsize=18)
else:
    root2 = Tk()
    root2.title('やり直し')
